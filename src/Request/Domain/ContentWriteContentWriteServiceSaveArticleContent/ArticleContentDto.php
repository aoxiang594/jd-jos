<?php
namespace Aoxiang\JdJos\Request\Domain\ContentWriteContentWriteServiceSaveArticleContent;
class ArticleContentDto{

    private $params=array();

    function __construct(){
        $this->params["@type"]="com.jd.content.write.client.domain.ArticleContentDto";
    }
    ...
    private $descriptionStrs;
                                        
    public function setDescriptionStrs($descriptionStrs){
        $size = count($descriptionStrs);
        for ($i=0; $i<$size; $i++){
            $descriptionStrs [$i] = $descriptionStrs [$i] ->getInstance();
        }
        $this->params['descriptionStrs'] = $descriptionStrs;
    }
