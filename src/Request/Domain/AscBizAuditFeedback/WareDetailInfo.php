...
class WareDetailInfo{
    ...
    private $pickWareVASList;
                                        
    public function setPickWareVASList($pickWareVASList){
        $size = count($pickWareVASList);
        for ($i=0; $i<$size; $i++){
            $pickWareVASList [$i] = $pickWareVASList [$i] ->getInstance();
        }
        $this->params['pickWareVASList'] = $pickWareVASList;
    }
